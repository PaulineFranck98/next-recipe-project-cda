...
const ArticleCard: React.FC<ArticleCardProps> = ({ article }) => {
  return (
    <div className='flex flex-col gap-2 group border border-slate-500 p-6 rounded-md hover:bg-slate-700 cursor-pointer hover:-translate-y-1 duration-300 h-[40vh]' key={ article.id }>
        <h2 className='text-xl font-bold'>{ article.title }</h2>
        <p className='text-sm text-slate-400'>{ formatDate(article.publicationDate)}</p>
